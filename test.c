...
bool position(PFILA f)
{
	...
	{
		if (i >= f->elementosNoHeap)
			break;
		if (f->heap[i]->posicao != i)
			return false;
	}
	return true;
}
...
bool isMaxHeap(PFILA f)
{
	...
	for (int i = 0; i < f->maxElementos; i++)
	{
		...
		{
			if (f->heap[i]->prioridade < f->heap[esq]->prioridade)
				return false;
...
int main()
{
	...
	if (inserirElemento(f, 11, 1))
	{
		...
		printf("5-ERRO (ESPERADO)\n");
	exibirLog(f);

	// Inserir em condições normais
	if (inserirElemento(f, 0, 1))
		printf("6-CORRETO\n");
	else
	{
		printf("6-incorreto\n");
