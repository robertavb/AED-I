...
void verificaEhAVL(PONT l)
{
  ...
  if (ehAVL2(l, &alt))
    ...
    printf("PROBLEMA: a arvore nao eh mais AVL.\n");
}
...
void usaTravessia(NO *l)
{
  ...
  scanf("%i", &ch);
  int niv;
  if (travessia(l, &niv, ch))
    printf("Elemento %i encontrado no nivel %i.\n", ch, niv);
  else
    printf("Nao foi possivel encontrar elemento %i.\n", ch);
}
